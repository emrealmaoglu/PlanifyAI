...
def test_level3_algorithm_integration():
    ...
    config = HSAGARunnerConfig(
        total_evaluations=300,  # Increased to ensure feasibility
        sa_fraction=0.3,
        sa_chains=2,
        population_size=20,     # Increased for 4 objectives
        parallel_sa=False,  # Disable parallel SA for tests (mocks not picklable)
        seed=42,
        verbose=True
    )
